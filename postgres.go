...
//
// NewPostgresAuditLog never applies graudit's schema itself: the
// graudit_entries table (and its indexes, internal/postgresdb/schema.sql)
// must already exist, applied through your own project's migration tool
// (golang-migrate, Flyway, a plain SQL file run in CI, ...) via
// PostgresSchemaSQL() before ever constructing an AuditLog — see
// PostgresSchemaSQL's own doc comment and docs/architecture.md's "Package
// flattened into one, GORM replaced with pgx/v5 + sqlc" section for why.
//
...
// be returned by PostgresSchemaSQL() and applied by graudit's own test
// setup. NewPostgresAuditLog itself never applies this — see
// PostgresSchemaSQL's own doc comment for why. The query code itself is
// generated by sqlc from the same directory (see internal/postgresdb) into
// its own internal package, isolating generated code from hand-written
// code.
//
//go:embed internal/postgresdb/schema.sql
var postgresSchemaSQL string

// PostgresSchemaSQL returns graudit's Postgres schema (plain CREATE
// TABLE/INDEX IF NOT EXISTS, no graudit-specific magic — see
// internal/postgresdb/schema.sql) as text, for the caller to apply through
// their own project's migration tool (golang-migrate, Flyway, a plain SQL
// file run in CI, whatever you already use) before constructing
// NewPostgresAuditLog.
//
// graudit deliberately never applies its own schema at runtime: doing so
...
// graudit and its schema changes. See docs/architecture.md's "Package
// flattened into one, GORM replaced with pgx/v5 + sqlc" section for the
...
// application (now only ever run by graudit's own test setup, never at
// connect time — see applyPostgresSchema's own doc comment), not per-Record
// serialization. See docs/architecture.md's "Postgres advisory-lock chain
// scoping" section.
...
// callers — e.g. multiple test binaries/processes racing to set up a fresh
// test database, where concurrent CREATE TABLE/INDEX IF NOT EXISTS
// statements are not fully race-free in Postgres. applyPostgresSchema is
// no longer called by NewPostgresAuditLog/connectPostgres (see their own
// doc comments) — it stays unexported and is used only by graudit's own
// test setup now, against a test database this process fully controls; a
// real consumer applies PostgresSchemaSQL() through their own migration
// tool instead. Distinct from gourdiantoken's (8_314_672_205), grnoti's
// (7_927_100_419), and grcache's (6_442_871_953) own lock keys, and from
// chainLockKey above, since these libraries may share a database.
...
// if set) and validates connectivity via Ping before returning.
//
// This never applies graudit's schema itself — see PostgresSchemaSQL. The
// graudit_entries table (and its indexes) must already exist before this
// is called, applied through your own project's migration tool. Calling
// this against a database where it doesn't exist yet doesn't fail here —
// it fails on the first actual AuditLog method call (Record, Query, ...)
// with a plain Postgres "relation does not exist" error.
//
// Parameters:
...
//     malformed, or the connection/Ping fails (wrapping
//     ErrBackendUnavailable)
//
// Example:
//
//	// PostgresSchemaSQL() applied via your own migration tool beforehand --
//	// see docs/architecture.md's "Package flattened into one, GORM replaced
//	// with pgx/v5 + sqlc" section.
...
// Ping, and returns whether the returned pool is owned by this call: true
// only when dialed from DSN here, false when cfg.Pool was supplied
// externally, since an externally-supplied pool is never graudit's to
// close (see postgresAuditLog.ownsPool and Close). Never applies graudit's
// schema — see PostgresSchemaSQL's own doc comment for why; the caller is
// responsible for the target database already having it applied (through
// their own migration tool) before calling NewPostgresAuditLog. Mirrors
// grnoti's own connectPostgres helper (grnoti/postgres.go) — that package
// has several Postgres-backed stores sharing this helper, hence the
// component parameter for error messages; graudit currently has only
...
//
// Unexported and no longer called by NewPostgresAuditLog/connectPostgres
// (see their own doc comments) — used only by graudit's own test setup
// now, against a test database this process fully controls. A real
// consumer applies PostgresSchemaSQL() through their own migration tool
// instead.
